...
for i in range(len(log_list)):
    if 'New connection from' in log_list[i]:
        ...
        has_malicious_command = any(x in command for x in ['curl', 'wget', 'sudo', 'chmod', 'useradd', 'ssh-keygen', 'netstat'])
        data.append((timestamp, ip_address, command, not_local, offtime, has_malicious_command))

df = pd.DataFrame(data, columns=['timestamp', 'ip_address', 'command', 'not_local', 'offtime', 'has_malicious_command'])
